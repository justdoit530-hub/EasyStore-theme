...
class VariantSelects {
  ...
  setUnavailable() {
    ...
    const priceEl = this.productInfo ? this.productInfo.querySelector('.price-item') : null;
    if (priceEl) priceEl.closest('.price').classList.add('visibility-hidden');
  }
...
class DeferredMedia extends HTMLElement {
  constructor() {
    ...
    if (!iframe) return;
    let youtube_id = iframe.getAttribute('data-video-id'),
        VID_REGEX = /(?:youtube(?:-nocookie)?\.com\/(?:[^\/\n\s]+\/\S+\/|(?:v|e(?:mbed)?)\/|\S*?[?&]v=)|youtu\.be\/)([a-zA-Z0-9_-]{11})/;
    if(youtube_id && youtube_id.match(VID_REGEX) != null) youtube_id = youtube_id.match(VID_REGEX)[1];
    if(youtube_id) iframe.src = `https://www.youtube.com/embed/${ youtube_id }?enablejsapi=1&rel=0`
  }
